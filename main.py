...
def main_events():
    ...
    for dex in DecentralizedExchange:
        ...
        for pair_created in new_pairs_filter.get_all_entries():
            ...
            pair = DexTradePair(
                dex=dex.value, token=token, creator_tx=get_tx(pair_created.transactionHash),
            ...
            print(f"Persisiting trades for pair {pair}...")
            for swap in swaps_filter.get_all_entries():
                swap_entry = get_DexTrade(swap, pair)
                db_manager.persist(swap_entry, commit=True)
